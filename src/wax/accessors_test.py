...
def test_unroll_dataset_accessor():
    ...
    with pytest.raises(ValueError):
        dataset.wax.stream().apply(module, format_dims=onp.array(["time", "lon", "lat"]))
...
def test_unroll_dataarray_accessor_module_same_shape(format):
    ...
    assert type(xoutput) is type(data)
...
def test_unroll_dataset_accessor_module_map():
    ...
    register_wax_accessors()
    dataset = prepare_test_data()
    xoutput, state = dataset.wax.stream(local_time="time").apply(module, format_dims=["time"])
    assert isinstance(xoutput, dict)
...
def test_air_temperature_dataframe():
    ...
    output = da.to_pandas().wax.stream(return_state=False).apply(diff, format_dims=da.dims)
    check = da.to_series().groupby(level=0).sum()
...
def test_air_temperature_series():
    ...
    output = (
        dataset.to_series().wax.stream(return_state=False).apply(diff, format_dims=dataset.dims)
    )
...
def test_ewm_dataframe():
    ...
    T = int(1.0e2)
    N = 10
    dataframe = pd.DataFrame(onp.random.normal(size=(T, N)), index=pd.date_range("1970", periods=T))
