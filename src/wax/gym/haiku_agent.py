...
class HaikuAgent(Agent):
    ...
    name: Any = None
    _seq: hk.PRNGSequence = field(init=False, default_factory=lambda: hk.PRNGSequence(42))
    ...
    def __call__(self, obs):
        ...
        action, self._state = self.agent.apply(self._params, self._state, next(self._seq), obs)
        return action
