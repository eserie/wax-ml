...
def test_init_value():
    x = jnp.ones((30,), "float64").at[0].set(jnp.nan).at[1].set(-1).at[5:20].set(jnp.nan)

    res = unroll(lambda x: EWMA(com=10, adjust=False, ignore_na=False)(x))(x)
    res_init0 = unroll(lambda x: EWMA(com=10, adjust=False, ignore_na=False, initial_value=0.0)(x))(
        x
    )
...
def test_train_ewma():
    ...
    def train():
        ...
        y_ref = unroll(lambda x: EWMA(com=COM_TARGET, adjust=True, ignore_na=False)(x))(x)
