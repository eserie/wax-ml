...
def test_mask_normalize_axis_1():
    ...
    for col in range(3):
        x_normalized_col_ref = jnp.nan_to_num(
            normalize(x[:, col][mask[:, col]]), posinf=0.0, neginf=0.0
        )
        # Use finite values and handle extreme cases more robustly
        x_norm_finite = jnp.where(jnp.isfinite(x_normalized[:, col]), x_normalized[:, col], 0.0)
        x_ref_finite = jnp.where(jnp.isfinite(x_normalized_col_ref), x_normalized_col_ref, 0.0)
        assert jnp.allclose(x_norm_finite, x_ref_finite, atol=1e-6)
