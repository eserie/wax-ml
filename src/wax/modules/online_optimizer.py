...
class OnlineOptimizer(hk.Module):
    ...
    def __init__(
        self,
        model: Callable | hk.TransformedWithState,
        opt: OptaxOptimizer | GradientTransformation,
        ...
        self.model = (
            model if isinstance(model, hk.TransformedWithState) else hk.transform_with_state(model)
        )
        self.opt = OptaxOptimizer(opt) if isinstance(opt, GradientTransformation) else opt
    ...
    def __call__(self, *args, **kwargs):
        ...
        def init_model_params_and_state(shape, dtype):
            ...
            trainable_params, non_trainable_params = partition(self.params_predicate, params)
        ...
        def _loss(trainable_params, non_trainable_params, state, *args, **kwargs):
            ...
            (loss, model_info), state = self.model.apply(params, state, None, *args, **kwargs)
        ...
        if self.project_params:
            updated_trainable_params = self.project_params(updated_trainable_params, self.opt)

        updated_params = hk.data_structures.merge(updated_trainable_params, non_trainable_params)
        step += 1
