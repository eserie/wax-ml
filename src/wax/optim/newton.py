...
import optax
from optax import ScalarOrSchedule, scale_by_learning_rate


def newton(learning_rate: ScalarOrSchedule, eps: float = 1e-5) -> optax.GradientTransformation:
...
def scale_by_newton(eps: float = 1e-7) -> optax.GradientTransformation:
    ...
    def update_fn(updates, state, params=None):
        ...
        updates = jax.tree_util.tree_map(lambda u, shape: u.reshape(shape), updates, shapes)
