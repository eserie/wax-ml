...
EncoderMapping = dict[Hashable, Callable[[Any], Encoder]] | dict[Hashable, Encoder]
...
def timestamp_is_before_local(
    ...
    return bool(secondary_timestamp < local_time_embedded)
...
def get_dataset_index_from_stream_index(
    ...
    for time_dim, idx in stream_index.items():
        ...
        if idx_value.ndim == 2:
            dataset[time_dim] = xr.DataArray(idx_value, dims=("step", time_dim + "_buffer_index"))
        else:
...
def _get_unique_time_dim(dataset):
    time_coords = {dim for dim, vals in dataset.coords.items() if vals.dtype.type is onp.datetime64}
...
def get_dataset_index(dataset: xr.Dataset, time_dataset_index: xr.Dataset) -> xr.Dataset:
...
def tree_buffer_transform(maxlen, fill_value, init_value):
    ...
    def transform(data):
        ...
        output = []
        for b, x, fv in zip(buffer, data, fill_value, strict=False):
            b.append(x)
...
def unroll_stream(
    stream: Generator, skip_first: bool = False, pbar: bool | Sequence[Any] = True
) -> Any:
    ...
    for obs in stream:
        ...
        for y, x in zip(outputs, obs_flat, strict=False):
            y.append(x)
...
class Stream:
    ...
    [`wax.modules.Buffer`](https://eserie.github.io/wax-ml/_autosummary/wax.modules.buffer.html#module-wax.modules.buffer))  # noqa
    ...
    *Terminology*: We use the name `local_time` to describe the time referential,
    ...
    def get_dataset_schema(
        xdata: xr.Dataset,
        types_encoders: EncoderMapping | None = None,
        check: bool = False,
    ) -> DatasetSchema:
    ...
    def prepare(
        ...
        dataset: xr.Dataset,
        module: Callable,
        encoders: EncoderMapping | None = None,
        skip_first=False,
    ...
    def merge(self, streams: dict[str, Generator[None, None, None]]) -> Generator[None, None, None]:
        ...
        while True:
            ...
            for time_dim, stream in streams.items():
                ...
                if time_dim in self.freqs:
                    ...
                    _is_before_local = partial(timestamp_is_before_local, embed, local_timestamp)
                else:
                ...
                if _is_verbose(self.verbose, time_dim):
                    ...
                    def as_list(vals, current_output, current_time_dim):
                        vals = onp.array(vals)
                        if vals.ndim == 2:
                            idx = onp.stack([vals[i] for i in range(len(vals))])
                            return pd.DataFrame(current_output[current_time_dim].value, index=idx)
                        idx = vals
