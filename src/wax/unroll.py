...
def unroll_transform_with_state(
    fun: Callable | TransformedWithState | UnrollTransformedWithState,
...
def unroll(
    ...
    if not isinstance(fun, UnrollTransformedWithState):
        fun = unroll_transform_with_state(fun, skip_first=skip_first, dynamic=dynamic, pbar=pbar)
...
def dynamic_unroll(
    fun: Callable | hk.TransformedWithState,
    params: Any,
    state: Any,
    rng: jnp.ndarray,
    skip_first: bool = False,
...
def static_scan(scan_f, init, xs=None, length=None, pbar=False):
    ...
    for x in iter_first_axis(xs, pbar=pbar):
        ...
        if ys is None:
            ys = tuple([] for _ in y)
        for ys_, y_ in zip(ys, y, strict=False):
            ys_.append(y_)
...
def dynamic_unroll_fori_loop(
    ...
    **kwargs,
    ...
    fun_init_params, fun_init_state = unroll_transform_with_state(fun).init(rng, *args, **kwargs)
    params = fun_init_params if params is None else params
    state = fun_init_state if state is None else state
    xs = (args, kwargs)
...
def data_unroll(data_gen, pbar=True):
    ...
    for obs, _info in data_gen:
        x = obs
        x, treedef = tree_flatten(x)
        if xs is None:
            # initialize xs
            xs = tuple([] for _ in x)
        for xs_, x_ in zip(xs, x, strict=False):
            xs_.append(x_)
    xs = (jnp.stack(x, axis=0) for x in xs)
...
def gym_static_unroll(gym_fun, params, state, seq, skip_first, data_generator, pbar=False):
    """Unroll a function over a data generator and random number generator.
    Args:
    ...
    for obs, _info in data_generator:
        y, state = gym_fun.apply(params, state, next(seq), obs)
        y, treedef = tree_flatten(y)
        if ys is None:
            ys = tuple([] for _ in y)
